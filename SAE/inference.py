...
def plot_multilingual_neuron_distribution(args):
    """
    Plots the distribution of the top 1% of multilingual neurons across all layers.
    """
    print("--- Starting Multilingual Neuron Distribution Plotting ---")
    ...
    def process_module(file_pattern, module_name, keys_to_process=None):
        ...
        for key_name, data_key in keys_to_process.items():
            ...
            if all_scores_tensor.numel() == 0: continue
        ...
        plt.title(f"Distribution of Top 1% Multilingual Neurons in {module_name} (Score Threshold: {threshold:.4f})", fontsize=16)
        plt.xlabel("Layer", fontsize=12)
        plt.ylabel("Count of Multilingual Neurons", fontsize=12)
        plt.xticks(layers)
        plt.legend(title="Component")
        plt.grid(True, which='both', linestyle='--', linewidth=0.5)
        plt.tight_layout()

        save_path_plot = os.path.join(plot_save_dir, f"{model_name_safe}_{module_name}_multilingual_neuron_distribution.png")
        plt.savefig(save_path_plot)
        plt.close()
        print(f"Plot for {module_name} saved to {save_path_plot}")

    process_module("layer_*_multilingual_neurons_mlp.pth", "MLP")
    process_module("layer_*_multilingual_neurons_attn.pth", "Attention", keys_to_process={'Q': 'avg_activations_q', 'K': 'avg_activations_k', 'V': 'avg_activations_v'})
